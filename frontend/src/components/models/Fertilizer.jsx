...
export default function Component() {
  ...
  const handlePredictClick = (e) => {
    e.preventDefault();
    // Use same-origin during dev (Vite will proxy /api to backend on 8080)
    ...
    const url = `${String(base).replace(/\/+$/, '')}/api/fertilizer`;
    setIsLoading(true);
    
    // Build payload (convert numerics where appropriate)
    const payload = {
      Temparature: formData.Temparature === '' ? null : Number(formData.Temparature),
      ...
      Phosphorous: formData.Phosphorous === '' ? null : Number(formData.Phosphorous),
    };
    ...
    }).filter(([, v]) => v === null || v === '' || (typeof v === 'number' && Number.isNaN(v))).map(([k]) => k);
    if (missing.length) {
      setIsLoading(false);
      setShowSpan(true);
      setResult('');
    ...
    fetch(url, {
      ...
      .then((response) => {
        ...
        // Normalize various backend formats e.g., "['Urea']", ["Urea"], or { Prediction: "Urea" }
        let pred = response?.Prediction ?? response?.prediction ?? response;
        ...
        let text = String(pred ?? '').trim();
        // Strip surrounding brackets/quotes like ['Urea'] -> Urea
        text = text.replace(/^\s*[\[\(\{\"\']+\s*/, '').replace(/\s*[\]\)\}\"\']+\s*$/, '');
        // If comma-separated, take the first token
        if (text.includes(',')) text = text.split(',')[0];
        text = text.replace(/^\s*[\"\']|[\"\']\s*$/g, '');
        setResult(text || 'No prediction');
        setIsLoading(false);
        setShowSpan(true);
        setErrorMsg("");
      })
      .catch((error) => {
        ...
        setShowSpan(true);
        setResult("");
        setErrorMsg(error?.message || "Error: Unable to predict fertilizer");
      });
  };
