...
export default function Component() {
  ...
  const handlePredictClick = (e) => {
    ...
    const base = isDev
      ...
      : (/^https?:\/\//i.test(envBase) && envBase.length > 0 ? envBase : window.location.origin)
    const url = `${String(base).replace(/\/+$/, '')}/api/crop`
    setIsLoading(true)
    ...
    // simple validation: require all fields and valid numbers for numeric
    const missing = [
      ['previousCrop', numericData['Previous Crop']],
      ['soilType', numericData['Soil Type']],
      ['moistureLevel', numericData['Moisture Level']],
      ['N', numericData['Nitrogen (N)']],
      ['P', numericData['Phosphorus (P)']],
      ['K', numericData['Potassium (K)']],
    ].filter(([k, v]) => v === '' || v === undefined || v === null || (typeof v === 'number' && Number.isNaN(v)))
     .map(([k]) => k)
    if (missing.length) {
      setIsLoading(false)
      setShowSpan(true)
    ...
    fetch(url, {
      ...
      .then((response) => {
        setResult(response['Recommended Crop'] || response.Prediction || response.recommendation || response.result || '')
        setIsLoading(false)
        setShowSpan(true)
        setErrorMsg("")
      })
