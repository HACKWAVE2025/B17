...
const CropRecommendation = () => {
    ...
    const [formData, setFormData] = useState({
        // Classic fields (kept for UI; not used by the new model)
        Nitrogen: '',
        Phosphorus: '',
        Potassium: '',
        Temperature: '',
    ...
    const handlePredictClick = (e) => {
        ...
        let url;
        if (isDev) {
            // In dev, always use same-origin and let Vite dev-server proxy /api to the backend
            url = `${window.location.origin.replace(/\/+$/, '')}/api/crop`;
        } else {
            const envBase = (import.meta.env.VITE_BACKEND_BASE_URL || '').trim();
            const base = /^https?:\/\//i.test(envBase) && envBase.length > 0 ? envBase : window.location.origin;
            url = `${String(base).replace(/\/+$/, '')}/api/crop`;
        }
        setIsloading(true);

        // Build payload for the current model
        const payload = {
            previousCrop: formData.previousCrop,
            soilType: formData.soilType,
            moistureLevel: formData.moistureLevel === '' ? null : Number(formData.moistureLevel),
            nitrogen: formData.Nitrogen === '' ? null : Number(formData.Nitrogen),
            phosphorus: formData.Phosphorus === '' ? null : Number(formData.Phosphorus),
            potassium: formData.Potassium === '' ? null : Number(formData.Potassium)
        };

        // Simple client-side validation to avoid sending empty values
        ...
        if (missing.length) {
            ...
            setShowSpan(true);
            setResult('');
            setErrorMsg(`Missing or invalid: ${missing.join(', ')}`);
            return;
        }
        ...
        fetch(url, {
            ...
            .then((response) => {
                // Accept both { Recommended Crop: "..." } and { Prediction: "..." }
                const recommended = response["Recommended Crop"] || response.Prediction || response.recommendation || response.result;
                        ...
                        <form method="post" acceptCharset="utf-8" name="Modelform" style={{ backgroundImage: `url(${bgHero})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                <div className="sm:col-span-2 flex justify-between items-center">
                                    <label className="font-semibold mr-2">Previous Crop</label>
                                    <select
                                    ...
                                    >
                                        {['Wheat','Maize','Millets','Groundnut','Cotton','Sorghum','Barley'].map(opt => (
                                ...
                                <div className="sm:col-span-2 flex justify-between items-center">
                                    ...
                                    <select
                                        ...
                                        onChange={handleChange}
                                    >
                                        {['Loamy','Clayey','Sandy','Saline'].map(opt => (
                                            <option key={opt} value={opt}>{opt}</option>
                                        ))}
                                    </select>
                                </div>
                                ...
                                {/* Legacy inputs below are kept for UI continuity but not used by the current model. */}
                            </div>
                            <div className="mb-4 mt-5">
                                <button type="button" className="w-full px-3 py-2 relative rounded group overflow-hidden font-medium bg-rose-50 text-rose-500 border border-rose-500 inline-block"
                                    disabled={isLoading}
                                ...
                                <h4 className="text-lg font-semibold">
                                    {result && Object.keys(result).length !== 0 ? (
                                        ...
                                        <div>
                                            ...
                                            {errorMsg && (
                                                <p className="mt-1 text-sm text-red-500 break-words">{errorMsg}</p>
                                            )}
                                        </div>
                                    )}
                                </h4>
