...
const SoilQuality = () => {
    ...
    const handlePredictClick = (e) => {
        e.preventDefault();
        // Use same-origin during local dev; in prod, prefer VITE_BACKEND_BASE_URL if valid
        ...
        const url = `${String(base).replace(/\/+$/, '')}/api/soil-quality`;
        setIsLoading(true);
        setErrorMsg("");
        setShowSpan(false);
        ...
        // Validate required fields (ensure none are NaN or empty)
        const missing = Object.entries(numericData)
        ...
        fetch(url, {
            ...
            .then((response) => {
                setResult(response.label || response.prediction || '');
                setIsLoading(false);
                setShowSpan(true);
                setErrorMsg("");
            })
                            ...
                            <div className="mt-4">
                                <h4 className="text-lg font-semibold ">
                                    {result ? (
                                        ...
                                        <p>{errorMsg || 'Please fill out each field in the form completely'}</p>
                                    )}
                                </h4>
                            </div>
                        )}
